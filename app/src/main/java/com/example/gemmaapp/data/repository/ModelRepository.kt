    ...
    fun startDownload() {
        val request = OneTimeWorkRequestBuilder<ModelDownloadWorker>()
            .setInputData(workDataOf(
                ModelDownloadWorker.KEY_DOWNLOAD_URL to GEMMA_4_E2B.downloadUrl,
                ModelDownloadWorker.KEY_FILE_NAME to GEMMA_4_E2B.fileName,
                ModelDownloadWorker.KEY_EXPECTED_SHA256 to GEMMA_4_E2B.sha256
            ))
            .setConstraints(
                Constraints.Builder()
        ...
        // KEEP if already running/enqueued; REPLACE if previously failed so retry works
        val policy = if (workManager.getWorkInfosForUniqueWork(WORK_NAME).get()
